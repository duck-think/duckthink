...
static string GenerateAskSql(ClientContext &context, const string &request, const string &tables_csv,
	...
	Value dm;
	bool have_manifest =
	    context.TryGetCurrentSetting("ask_semantic_manifest", dm) && !dm.IsNull() && !dm.ToString().empty();

	// Domain scoping: SET ask_scope_tags='taxi' pulls in every model/source carrying that
	...
	Value stv;
	if (have_manifest && context.TryGetCurrentSetting("ask_scope_tags", stv) && !stv.IsNull() &&
	    !stv.ToString().empty()) {
		std::set<string> tags;
	...
	if (table_list.empty()) {
		throw BinderException("ASK: no tables in scope — the database has no tables, or list them in ON (...) / "
		                      "SET ask_scope_tags / SET ask_semantic_manifest.");
	}
