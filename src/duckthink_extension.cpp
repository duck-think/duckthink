...
static ParserOverrideResult DuckthinkParserOverride(ParserExtensionInfo *info, const string &query,
	...
	for (int guard = 0; guard < 32; guard++) {
		...
		for (size_t j = 0; j + 1 < toks.size(); j++) {
			...
			// ask_scope_tags (a domain-scoped ASK).
...
static void LoadInternal(ExtensionLoader &loader) {
	...
	config.AddExtensionOption("ask_semantic_manifest",
	...
	config.AddExtensionOption("ask_scope_tags",
	                          "Comma-separated dbt tags. When set (with ask_semantic_manifest), ASK() scopes to "
