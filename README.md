...
```sql
SET ask_semantic_manifest = '/path/to/dbt/target/manifest.json';
...
```sql
SET ask_scope_tags = 'taxi';           -- every model/source tagged `taxi`
...
2. the tables a **`ask_scope_tags`** value resolves to (needs a manifest), or
...
| `ask_semantic_manifest` | `''` | path to a dbt `manifest.json` / `semantic_manifest.json`; empty = off |
| `ask_scope_tags` | `''` | comma-separated dbt tags: scope `ASK` to a **domain** instead of naming tables (see below) |
